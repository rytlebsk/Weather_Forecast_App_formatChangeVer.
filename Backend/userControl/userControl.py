...
def login():
    ...
    if userAccount == None or password == None:
        dt = UserDataResult(
            "-1", "", "", "Index Error. Please Follow Documentaion Instructions")
        response = make_response(jsonify(dt.to_dict()), 404)
        return response
    cursor.execute(
        "SELECT * FROM users WHERE account =? and password =?", (userAccount, password))
    user = cursor.fetchall()
    if len(user) == 0:
...
def register():
    ...
    if request.method == 'POST':
        ...
        if len(user) > 0:
            ...
            return response
        cursor.execute(
            "INSERT INTO users(account,password,deleted) VALUES(?,?,?)", (userAccount, password, 0))
        conn.commit()
        cursor.execute(
            "SELECT * FROM users WHERE account =? and password =?", (userAccount, password))
        user = cursor.fetchall()
        if (len(user) != 0):
            dt = UserDataResult(user[0][0], user[0][1],
                                user[0][2], "Register Successful")
...
def getDailySportsSuggestion():
    ...
    final_dict = {}
    dressing_dict = [
        {"name": "衣著", "suggestion": handler.getDressingSuggestion()}]
    health_dict = [{"name": "健康", "suggestion": handler.getHealthSuggestion()}]
    tansportation_dict = [
        {"name": "交通", "suggestion": handler.getTransportationSuggestion()}]
